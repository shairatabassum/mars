...
"""
Zip-NeRF implementation for MARS following Nerfstudio. Author: Shaira Tabassum
"""
...
from mars.fields.zipnerf_field import ZipNeRFField
from mars.model_components.zipnerf_samplers import PowerSampler, zipnerf_proposal_loss
...
class ZipNeRFModelConfig(ModelConfig):
    ...
    proposal_net_args_list: List[Dict] = field(
        default_factory=lambda: [
            ...
            {"hidden_dim": 16, "log2_hashmap_size": 17, "num_levels": 5, "max_res": 2048, "use_linear": False},
        ]
    )
    ...
    quantized_density_mult: float = 0.0001
    """Quantized density loss multiplier."""
    weight_clipping_loss_mult: float = 0.01
    """Weight clipping loss multiplier. Makes sure nothing is out of bounds for numerical overflow reasons"""
    ...
    use_emittance: bool = False


class ZipNeRFModel(Model):
    """ZipNeRF model following Nerfacto
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        outputs["quantized_density_loss"] = torch.minimum(
            -torch.log(field_outputs[FieldHeadNames.DENSITY]),
            -torch.log(1 - field_outputs[FieldHeadNames.DENSITY]),
        ).mean(dim=-2)
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if self.training:
            loss_dict["interlevel_loss"] = self.config.interlevel_loss_mult * zipnerf_proposal_loss(
            ...
            # loss_dict["weight_clipping_loss"] = self.config.weight_clipping_loss_mult * torch.mean(
            #     outputs["weight_clipping_loss"]
            # )
        return loss_dict
