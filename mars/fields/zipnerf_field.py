...
class ZipNeRFField(Field):
    """Compound Field that uses TCNN: For ZipNeRF
    ...
    def __init__(
        ...
        self.mlp_head = tcnn.Network(
            n_input_dims=self.direction_encoding.n_output_dims + self.geo_feat_dim + self.appearance_embedding_dim + self.obj_feat_dim,
            n_output_dims=3,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[Tensor, Tensor]:
        ...
        x = (1 / (2.82842712474619 * standard_deviations * nl))
        multisample_weights = 2 * torch.sign(x) * torch.sqrt(1 - torch.exp(-(4 / torch.pi) * (x**2))) # 1 / sqrt(8 * std**2 * nl**2)
