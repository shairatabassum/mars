...
def world2object(pts, dirs, pose, theta_y, dim=None, inverse=False):
    ...
    x_shift = (
        torch.tensor([-1.0, 0.0, 0.0]).unsqueeze(0)
        if inverse
        else torch.tensor([-1.0, 0.0, 0.0]).unsqueeze(0).unsqueeze(0)
    ).to(dim.device) * (dim[..., 0] / 2).unsqueeze(-1)
...
def object2world(pts, dirs, pose, theta_y, dim=None, inverse=True):
    ...
    # Shift the object reference point to the middle of the bbox (vkitti2 specific)
    x_shift = torch.tensor([-1.0, 0.0, 0.0]).unsqueeze(0).to(dim.device) * (dim[..., 0] / 2).unsqueeze(-1)
